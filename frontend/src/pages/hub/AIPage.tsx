...
import { LuBot, LuMessageSquare, LuSettings2, LuSparkles } from 'react-icons/lu'
...
const AIPage = () => {
    ...
    return (
        <HubPage
            ...
            actions={<Button asChild variant='soft'><a href='/app/raven-bot' target='_blank' rel='noreferrer'>Manage agents <LuSettings2 /></a></Button>}
        >
            <ErrorBanner error={error} />
            <Flex align='center' gap='2' mb='4'>
                <LuSparkles className='text-accent-11' />
                ...
                {bots.map((bot) => (
                    <Box key={bot.name} className='rounded-xl border border-gray-4 bg-gray-1 p-5 dark:border-gray-6 dark:bg-gray-1'>
                        <Flex align='start' justify='between' gap='3'>
                            ...
                            <Flex align='center' justify='center' className='h-8 w-8 rounded-full bg-accent-3 text-accent-11'><LuBot /></Flex>
                        </Flex>
                        ...
                        <Button onClick={() => openBot(bot.name)} disabled={opening === bot.name} className='w-full'>
                            {opening === bot.name ? <Loader /> : <LuMessageSquare />} Chat
                        </Button>
                    </Box>
                ))}
            </Grid>
            {bots.length === 0 && (
                <Flex direction='column' align='center' justify='center' className='min-h-72 rounded-xl border border-dashed border-gray-6 px-6 text-center'>
                    <LuBot size={36} className='mb-3 text-gray-8' />
