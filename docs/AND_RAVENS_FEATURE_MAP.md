...
| Channel tabs | Built-in Posts and Shared tabs on every workspace channel, with Shared backed by channel file APIs | Raven messages + files |
...
| Configurable additional channel tabs (Page, Notes, Planner, Power BI, custom apps) | Built-in Posts and Shared are implemented. Additional configurable tabs still require a `Raven Channel Tab` model, app manifest/permission model, tab configuration UI and safe embed policy. |
